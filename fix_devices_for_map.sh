#!/bin/bash
# Fix DK-RP-2 and SB1-RP-2: Activate them and ensure they have location data

echo "=========================================="
echo "FIXING DEVICES FOR MAP (DK-RP-2 and SB1-RP-2)"
echo "=========================================="

# Fix DK-RP-2
echo ""
echo "Fixing DK-RP-2 (Digital Kiosk 2)..."
...
SELECT 
    ...
    true,
    false,
    '{\"access_token\": \"murraba\", \"mqtt_topic\": \"device/DK-RP-2/telemetry\"}'
FROM ref_device
ON CONFLICT (device_id) 
DO UPDATE SET
    is_active = true,
    device_metadata = '{\"access_token\": \"murraba\", \"mqtt_topic\": \"device/DK-RP-2/telemetry\"}',
...
docker exec iot-postgres psql -U iot_user -d iot_platform -c "SELECT d.device_id, d.name, d.is_active FROM devices d WHERE d.device_id IN ('DK-RP-2', 'SB1-RP-2');"
...
echo ""
echo "Next steps:"
echo "1. Start DK-RP-2 simulator:"
echo "   cd /home/mishal/my-iot-project && source venv/bin/activate"
echo "   MQTT_HOST=localhost MQTT_PORT=1884 ACCESS_TOKEN=murraba python3 scripts/digital_kiosk_dkrp2_sim.py &"
echo ""
echo "2. Start SB1-RP-2 simulator:"
