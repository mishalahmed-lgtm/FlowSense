...
# 1. E-Scooter (ES-RP-1)
nohup python simulators/escooter_esrp1_sim.py > /tmp/escooter_esrp1.log 2>&1 &
echo "  ✓ ES-RP-1 (E-Scooter) - PID: $!"

# 2. Smart Bin (SB-RP-1)
DEVICE_ID=SB-RP-1 nohup python simulators/smart_bin_sim.py > /tmp/smart_bin_sb.log 2>&1 &
echo "  ✓ SB-RP-1 (Smart Bin) - PID: $!"

# 3. Fitness Rower (SG-RP-1)
nohup python simulators/fitness_sgrp1_sim.py > /tmp/fitness_sgrp1.log 2>&1 &
echo "  ✓ SG-RP-1 (Fitness Rower) - PID: $!"

# 4. Noise Sensor (AN-RP-1)
nohup python simulators/noise_anrp1_sim.py > /tmp/noise_anrp1.log 2>&1 &
echo "  ✓ AN-RP-1 (Noise Sensor) - PID: $!"

# 5. Digital Kiosk (DK_MP-1)
KIOSK_DEVICE_ID=DK_MP-1 nohup python simulators/digital_kiosk_sim.py > /tmp/digital_kiosk.log 2>&1 &
echo "  ✓ DK_MP-1 (Digital Kiosk) - PID: $!"

# 6. Smart Bench (SM1-RP)
BENCH_ID=SM1-RP nohup python simulators/smart_bench_sim.py > /tmp/smart_bench.log 2>&1 &
echo "  ✓ SM1-RP (Smart Bench) - PID: $!"

# 7. Pathway Light (PLP-RP-1)
DEVICE_ID=PLP-RP-1 nohup python simulators/lightpole_plprp1_sim.py > /tmp/lightpole_plprp1.log 2>&1 &
echo "  ✓ PLP-RP-1 (Pathway Light) - PID: $!"

# 8. Master Water Sensor (MW-RP-1)
DEVICE_ID=MW-RP-1 ACCESS_TOKEN=murabba nohup python simulators/master_water_sensor_sim.py > /tmp/master_water_sensor.log 2>&1 &
