#!/bin/bash
# Fix SB1-RP-2 registration and run the simulator

echo "=========================================="
echo "FIXING SB1-RP-2 DEVICE REGISTRATION"
echo "=========================================="

# Fix SB1-RP-2 in database
...
echo ""
echo "Verifying SB1-RP-2 configuration..."
docker exec iot-postgres psql -U iot_user -d iot_platform -c "SELECT d.device_id, d.name, d.is_active, d.device_metadata FROM devices d WHERE d.device_id = 'SB1-RP-2';"

echo ""
echo "=========================================="
echo "STARTING SB1-RP-2 SIMULATOR"
echo "=========================================="

cd "$(dirname "$0")"
source venv/bin/activate

MQTT_HOST=localhost MQTT_PORT=1884 ACCESS_TOKEN=murraba python3 scripts/smartbench_sb1rp2_sim.py
